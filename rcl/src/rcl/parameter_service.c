  ...
  const rcl_parameter_service_t * parameter_service)
{
  ...
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add get_parameters service to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_service(wait_set, &parameter_service->impl->get_types_service);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add get_parameter_types service to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_service(wait_set, &parameter_service->impl->set_service);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add set_parameters service to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_service(wait_set, &parameter_service->impl->set_atomically_service);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add set_parameters_atomically service to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_service(wait_set, &parameter_service->impl->list_service);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add list_parameters service to waitset!");
    return ret;
  }
