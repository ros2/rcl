...
#define RCL_PARAMETER_INITIALIZE_CLIENT(VERB, SRV_TYPE_NAME, SRV_SUFFIX) \
  { \
    const rosidl_service_type_support_t * VERB ## _ts = ROSIDL_GET_TYPE_SUPPORT( \
...
)
{
  ...
  const rosidl_message_type_support_t * event_ts = ROSIDL_GET_TYPE_SUPPORT(
  ...
  const rcl_parameter_client_t * parameter_client)
{
  ...
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add get_parameters client to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_client(wait_set, &parameter_client->impl->get_types_client);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add get_parameter_types client to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_client(wait_set, &parameter_client->impl->set_client);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add set_parameters client to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_client(wait_set, &parameter_client->impl->set_atomically_client);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add set_parameters_atomically client to waitset!");
    return ret;
  }
  ret = rcl_wait_set_add_client(wait_set, &parameter_client->impl->list_client);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add list_parameters client to waitset!");
    return ret;
  }

  ret = rcl_wait_set_add_subscription(wait_set, &parameter_client->impl->event_subscription);
  if (ret != RCL_RET_OK) {
    RCL_SET_ERROR_MSG("Failed to add parameter events subscription to waitset!");
    return ret;
  }
