...
public:
  ...
  void SetUp() override
  {
    ...
    ret = rcl_timer_init(
      ...
      rcl_get_default_timer_options(), rcl_get_default_allocator());
    ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  }
...
TEST_F(TestTimerFixture, test_timer_init_with_invalid_arguments) {
  ...
  ret = rcl_timer_init(
    nullptr, &clock, this->context_ptr, RCL_MS_TO_NS(50), nullptr,
    rcl_get_default_timer_options(), allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  rcl_reset_error();

  ret = rcl_timer_init(
    &timer, nullptr, this->context_ptr, RCL_MS_TO_NS(50), nullptr,
    rcl_get_default_timer_options(), allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  rcl_reset_error();

  ret = rcl_timer_init(
    &timer, &clock, nullptr, RCL_MS_TO_NS(50), nullptr,
    rcl_get_default_timer_options(), allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  rcl_reset_error();

  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, -1, nullptr,
    rcl_get_default_timer_options(), allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  rcl_reset_error();
  ...
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, RCL_MS_TO_NS(50), nullptr,
    rcl_get_default_timer_options(), invalid_allocator);
  EXPECT_EQ(RCL_RET_INVALID_ARGUMENT, ret);
  rcl_reset_error();
}
...
TEST_F(TestTimerFixture, test_timer_with_invalid_clock) {
  ...
  rcl_timer_t timer = rcl_get_zero_initialized_timer();
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, 0, nullptr,
    rcl_get_default_timer_options(), allocator);
...
TEST_F(TestTimerFixture, test_two_timers) {
  ...
  ret = rcl_timer_init(
    &timer2, &clock, this->context_ptr, RCL_MS_TO_NS(1000), nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(TestTimerFixture, test_two_timers_ready_before_timeout) {
  ...
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, RCL_MS_TO_NS(10), nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(TestTimerFixture, test_timer_not_ready) {
  ...
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, RCL_MS_TO_NS(1000), nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
...
TEST_F(TestTimerFixture, test_timer_overrun) {
  ...
  rcl_timer_t timer = rcl_get_zero_initialized_timer();
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, RCL_MS_TO_NS(200), nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT(
  {
...
TEST_F(TestTimerFixture, test_rostime_time_until_next_call) {
  ...
  rcl_timer_t timer = rcl_get_zero_initialized_timer();
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, sec_5, nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT(
  {
...
TEST_F(TestTimerFixture, test_ros_time_wakes_wait) {
  ...
  rcl_timer_t timer = rcl_get_zero_initialized_timer();
  ret = rcl_timer_init(
    &timer, &clock, this->context_ptr, sec_1, nullptr,
    rcl_get_default_timer_options(), rcl_get_default_allocator());
  ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  OSRF_TESTING_TOOLS_CPP_SCOPE_EXIT(
  {
...
TEST_F(TestPreInitTimer, test_invalid_init_fini) {
  ...
  ASSERT_EQ(
    RCL_RET_BAD_ALLOC, rcl_timer_init(
      &timer_fail, &clock, this->context_ptr, RCL_S_TO_NS(1),
      timer_callback_test, rcl_get_default_timer_options(),
