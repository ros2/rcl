...
if(rcl_add_custom_gmock_INCLUDED)
  return()
endif()
set(rcl_add_custom_gmock_INCLUDED TRUE)

#
# Custom macro for adding a gmock in rcl.
#
# It also takes some of the arguments of ament_add_gmock as well as
...
# :type target: string
# :param SRCS: list of source files used to create the gmock
...
# @public
#
macro(rcl_add_custom_gmock target)
  ...
  if(_ARG_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "rcl_add_custom_gmock() called with unused arguments: ${_ARG_UNPARSED_ARGUMENTS}")
  endif()
  if(_ARG_ENV)
  ...
  # Pass args along to ament_add_gmock().
  ament_add_gmock(${target} ${_ARG_SRCS} ${_ARG_ENV} ${_ARG_APPEND_ENV} ${_ARG_APPEND_LIBRARY_DIRS} ${_ARG_SKIP_TEST})
  # Check if the target was actually created.
  if(TARGET ${target})
    if(_ARG_TRACE)
      message(STATUS "rcl_add_custom_gmock() Target '${target}':")
    endif()
    # Add extra include directories, if any.
    if(_ARG_INCLUDE_DIRS)
      if(_ARG_TRACE)
        message(STATUS "  rcl_add_custom_gmock() INCLUDE_DIRS: ${_ARG_INCLUDE_DIRS}")
      endif()
    ...
    if(_ARG_LIBRARIES)
      if(_ARG_TRACE)
        message(STATUS "  rcl_add_custom_gmock() LIBRARIES: ${_ARG_LIBRARIES}")
      endif()
    ...
    if(_ARG_AMENT_DEPENDENCIES)
      if(_ARG_TRACE)
        message(STATUS "  rcl_add_custom_gmock() AMENT_DEPENDENCIES: ${_ARG_AMENT_DEPENDENCIES}")
      endif()
