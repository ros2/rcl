...
{
  return node_params_init_with_capacity(node_params, allocator, INIT_NUM_PARAMS_PER_NODE);
}
  ...
  size_t new_capacity)
{
  ...
  RCUTILS_CHECK_ALLOCATOR_WITH_MSG(
    &allocator, "invalid allocator", return RCUTILS_RET_INVALID_ARGUMENT);
  // invalid if new_capacity is smaller than num_params
  ...
  if (NULL == parameter_values) {
    allocator.deallocate(parameter_names, allocator.state);
    return RCUTILS_RET_BAD_ALLOC;
  }
